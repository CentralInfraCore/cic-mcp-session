#!/usr/bin/env python3
"""Claude Code `PostToolUseFailure` hook -> SANDBOXED session ingest outbox.

Job: session-ingest-hook-sandboxed-001. Thin entry-point: all logic lives in
hooks/_ingest_sandbox.py:run_hook(). Fires after a failed tool call; enqueues the event envelope.
...
COLLECTOR_NAME = "PostToolUseFailure.py"
