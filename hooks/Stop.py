#!/usr/bin/env python3
"""Claude Code `Stop` hook -> SANDBOXED session ingest outbox.

Job: session-ingest-hook-sandboxed-001. Thin entry-point: all logic lives in
hooks/_ingest_sandbox.py:run_hook(). Fires when the assistant stops; enqueues the event envelope AND incrementally reads + enqueues transcript turns via session_store.transcript_reader.
...
COLLECTOR_NAME = "Stop.py"

if __name__ == "__main__":
    sys.exit(run_hook(COLLECTOR_NAME, extract_turns=True))
